// Разблокировка личных сообщений: вход по токену (без пароля) не позволяет
...
async function unlock() {
  ...
  busy.value = true
  try {
    await channels.submitKek(password.value)
    if (!channels.kekPromptVisible) password.value = ''
  } finally {
    busy.value = false
  }
}
...
<template>
  <Dialog :open="channels.kekPromptVisible" @update:open="onOpenChange">
    <DialogContent class="max-w-[420px]">
      <DialogHeader>
        <DialogTitle class="text-center">Разблокировать личные сообщения</DialogTitle>
      </DialogHeader>
      <p class="text-sm text-muted-foreground">
        Для расшифровки личных сообщений и приватных каналов введите пароль аккаунта.
        Понадобится один раз — в дальнейшем ключ будет восстановлен автоматически.
      </p>
      <input
        v-model="password"
        type="password"
