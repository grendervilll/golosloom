...
;(async () => {
  ...
  await p1.fill('input[placeholder="Название канала"]', 'final-' + ts)
  ...
  // User2 входит в канал — ищем по creator_id
  const user2ChannelId = await p2.evaluate(async (creatorId) => {
    const t = localStorage.getItem('golosloom-token')
    const chs = await fetch('/api/channels', { headers: { Authorization: 'Bearer ' + t } }).then(r => r.json())
    const ch = chs.find(c => c.is_member && c.name.startsWith('final-'))
    return ch ? ch.id : null
  })
  console.log('User2 found channel:', user2ChannelId)
  
  if (user2ChannelId) {
    // Кликаем на канал в sidebar
    await p2.locator('.chat-list .chat-row').filter({ hasText: 'final-' }).first().click()
    ...
    await p1.waitForTimeout(3000)
    await p1.locator('.chat-list .chat-row').filter({ hasText: 'final-' }).first().click()
    await p1.waitForTimeout(3000)
    ...
    ok('5. User2 канал', false, 'channel not found')
    ok('6. User2 отправил', false, 'skipped')
    ok('7. testuser1 видит', false, 'skipped')
  }
