...
;(async () => {
  ...
  await p1.getByPlaceholder('Ваш ник').fill('u1v2_' + ts)
  ...
  await p1.fill('input[placeholder="Название канала"]', 'chv2-' + ts)
  ...
  const chId = await p1.evaluate(async () => {
    ...
    return chs.find(c => c.is_member && c.name.startsWith('chv2-'))?.id
  })
  ...
  await p2.getByPlaceholder('Ваш ник').fill('u2v2_' + ts)
  ...
  // Click channel
  await p2.locator('.chat-list .chat-row').filter({ hasText: 'chv2-' }).first().click()
