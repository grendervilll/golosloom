...
;(async () => {
  ...
  await p1.fill('input[placeholder="Название канала"]', 'E2E-' + Date.now())
  ...
  await p1.waitForTimeout(3000)
  const m1 = await p1.evaluate(async (id) => {
    const t = localStorage.getItem('golosloom-token')
    return (await fetch('/api/channels/' + id + '/messages', { headers: { Authorization: 'Bearer ' + t } }).then(r => r.json())).length
  }, chId)
  ok('3. Сообщение testuser1', m1 > 0, `msgs=${m1}`)

  // User 2: новый пользователь
  const nick2 = 'v_' + Date.now().toString().slice(-6)
  ...
  // User2 входит в канал
  await p2.locator('.chat-list .chat-row').filter({ hasText: 'E2E-' }).first().click()
  await p2.waitForTimeout(5000)
  const m2 = await p2.evaluate(async (id) => {
    const t = localStorage.getItem('golosloom-token')
    return (await fetch('/api/channels/' + id + '/messages', { headers: { Authorization: 'Bearer ' + t } }).then(r => r.json())).length
  }, chId)
  ok('5. User2 видит сообщение testuser1', m2 > 0, `msgs=${m2}`)

  // User2 отправляет ответ
  ...
  // testuser1 обновляет страницу и видит ответ
  ...
  ok('7. testuser1 видит ответ User2 (после обновления)', m4 > 1, `msgs=${m4}`)

  // UI проверка
  const ui = await p1.locator('.chat-panel .msg').count()
  ok('8. UI показывает сообщения', ui > 0, `ui=${ui}`)
