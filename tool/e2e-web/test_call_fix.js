...
;(async () => {
  ...
  await p1.getByPlaceholder('Ваш ник').fill('cf_' + ts)
  ...
  p2.on('console', m => { if (m.type() === 'error') console.log('[p2:err]', m.text().slice(0, 200)) })
  ...
  await p1.fill('input[placeholder="Название канала"]', 'callfix-' + ts)
  ...
  const chId = await p1.evaluate(async () => {
    ...
    return chs.find(c => c.is_member && c.name.startsWith('callfix-'))?.id
  })
