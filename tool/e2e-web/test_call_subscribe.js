...
;(async () => {
  ...
  await p1.getByPlaceholder('Ваш ник').fill('cs1_' + ts)
  ...
  await p2.getByPlaceholder('Ваш ник').fill('cs2_' + ts)
  ...
  await p1.fill('input[placeholder="Название канала"]', 'csc-' + ts)
  ...
  const chId = await p1.evaluate(async () => {
    ...
    return chs.find(c => c.is_member && c.name.startsWith('csc-'))?.id
  })
