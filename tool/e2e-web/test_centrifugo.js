...
;(async () => {
  ...
  p.on('console', m => {
    const t = m.text()
    if (t.includes('centrifuge') || t.includes('subscribe') || t.includes('user:') || t.includes('connect'))
  ...
  await p.getByPlaceholder('Ваш ник').fill('ct_' + ts)
  ...
  // Wait for Centrifuge connection
