...
;(async () => {
  ...
  // User2 входит в канал — ищем по имени
  await p2.locator('.chat-list .chat-row').filter({ hasText: 'TEST-' }).first().click()
  await p2.waitForTimeout(5000)

  // Проверяем что User2 в правильном канале
  const user2Channel = await p2.evaluate(async () => {
    const t = localStorage.getItem('golosloom-token')
    const me = await fetch('/api/me', { headers: { Authorization: 'Bearer ' + t } }).then(r => r.json())
    const chs = await fetch('/api/channels', { headers: { Authorization: 'Bearer ' + t } }).then(r => r.json())
    const mine = chs.filter(c => c.is_member)
    return mine.map(c => ({ id: c.id, name: c.name }))
  })
  console.log('User2 channels:', JSON.stringify(user2Channel))
