...
;(async () => {
  ...
  p1.on('console', m => { const t = m.text(); if (t.includes('auth') || t.includes('centrifuge') || t.includes('subscribe')) console.log('[p1]', t.slice(0, 200)) })
  await p1.goto(BASE, { waitUntil: 'load', timeout: 30000 })
  await p1.waitForTimeout(2000)
  await p1.getByText('Зарегистрироваться').first().click()
  await p1.getByPlaceholder('Ваш ник').fill('fl1_' + ts)
  ...
  p2.on('console', m => { const t = m.text(); if (t.includes('auth') || t.includes('centrifuge') || t.includes('subscribe') || t.includes('calls') || t.includes('channels')) console.log('[p2]', t.slice(0, 200)) })
  await p2.goto(BASE, { waitUntil: 'load', timeout: 30000 })
  await p2.waitForTimeout(2000)
  await p2.getByText('Зарегистрироваться').first().click()
  await p2.getByPlaceholder('Ваш ник').fill('fl2_' + ts)
  ...
  await p1.fill('input[placeholder="Название канала"]', 'fl-' + ts)
  ...
  const chId = await p1.evaluate(async () => {
    ...
    return chs.find(c => c.is_member && c.name.startsWith('fl-'))?.id
  })
  ...
  // User2 открывает канал (это запускает polling звонков)
