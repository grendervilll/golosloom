// Смок-тест звонков: A создаёт звонок в DM, B принимает, оба подключаются
// к LiveKit (синтетический аудио), проверяем обмен SDP (offer→answer,
// subscriber offer) — это доказывает работу сигналинга и WebRTC.
...
void runCallSmoke(const QString& server) {
  ...
  qint64 dmId = 0;
  qint64 bUserId = 0;
  addStep([a, b, nickB, &dmId, &bUserId](auto next) {
    a->api()->search(nickB, [a, b, nickB, &dmId, &bUserId, next](const QJsonObject& res, const QString& err) {
      if (!err.isEmpty()) return failStep2("поиск: " + err);
      const QJsonArray users = res.value("users").toArray();
      if (users.isEmpty()) return failStep2("пользователь не найден");
      const qint64 uid = users.first().toObject().value("id").toVariant().toLongLong();
      bUserId = uid;
      a->createDm(uid, [a, b, &dmId, next](const QString& err2) {
        if (!err2.isEmpty()) return failStep2("DM: " + err2);
        QTimer::singleShot(1000, [a, b, &dmId, next]() {
          b->refreshChannelsPublic();
          QStringList names;
          for (const Channel& c : a->channels()) names << (QString::number(c.id) + ":" + c.kind);
          qInfo() << "  A channels:" << names.join(", ");
          for (const Channel& c : a->channels()) {
            if (c.kind == "dm") dmId = c.id;
          }
          next();
        });
      });
  ...
  addStep([cmA, cmB, &dmId, &bUserId](auto next) {
    ok("DM создан (id=" + QString::number(dmId) + ")", dmId > 0);
    // A звонит B (в целях — B).
    cmA->startCall(dmId, {bUserId});
