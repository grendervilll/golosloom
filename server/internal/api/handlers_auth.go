...
func (s *Server) handleUploadKey(w http.ResponseWriter, r *http.Request) {
	...
	// Чистим старые устройства: каждая перезагрузка страницы создаёт новое,
	// и накопление «мёртвых» устройств заставляет клиенты заново оборачивать
	// ключи каналов для каждого из них (десятки медленных запросов).
	_ = s.Store.PruneDevices(userIDFrom(r), 8)
